...
export function Preview({ preview, selectedItem, selectedType }: PreviewProps) {
  ...
  const { isAccessibilityEnabled, isShelfOpen, isShelfPinned } = useUIStore()
  const isShelfVisible = isShelfOpen || isShelfPinned

  if (!preview) {
    return (
      <div className={previewStyles.previewContainer} data-shelf-open={isShelfVisible}>
